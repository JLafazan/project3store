
import Layout from '../components/Layout'



const IntroductionPage = () => (
  <Layout title="intro">

    <meta charset="utf-8" />

    <h1>VineQuest</h1>

    <h3>Castello di Amorosa,  Darioush, and Sterling Wineries</h3>
    <div id="video" class="side-by-side">
      <iframe width="574" height="290" src="https://www.youtube.com/embed/FPrAJ7yqoiI" frameborder="0" allow="autoplay; encrypted-media"
        align="middle" allowfullscreen></iframe>
    </div>
    <h1>
      <p id="rumi">"Either give me more wine or leave me alone." -Rumi</p>
    </h1>

    <h3>Rombauer Winery</h3>
    <div id="video" class="side-by-side">
      <iframe width="574" height="290" src="https://youtube.com/embed/uTURlfz_rD4" frameborder="0" allow="autoplay; encrypted-media"
        align="middle" allowfullscreen></iframe>
    </div>
    <h1>
      <p id="rumi">"Either give me more wine or leave me alone." -Rumi</p>
    </h1>

    <h3>Robert Mondavi Winery</h3>
    <div id="video" class="side-by-side">
      <iframe width="574" height="290" src="https://youtube.com/embed/GeFgrOTxZXw" frameborder="0" allow="autoplay; encrypted-media"
        align="middle" allowfullscreen></iframe>
    </div>
    <h1>
      <p id="rumi">"Either give me more wine or leave me alone." -Rumi</p>
    </h1>
  </Layout>

)

export default IntroductionPage
